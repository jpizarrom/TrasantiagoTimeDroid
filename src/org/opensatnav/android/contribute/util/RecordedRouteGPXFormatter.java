...
public class RecordedRouteGPXFormatter implements OSMConstants{
	 ...
	 * 
	 */
	public static String create(final ArrayList<Location> someRecords, String username) throws IllegalArgumentException {
		if(someRecords == null || someRecords.size() == 0)
			throw new IllegalArgumentException();
		
			
		final StringBuilder sb = new StringBuilder();
		final Formatter f = new Formatter(sb,java.util.Locale.US);
		...
		f.format(GPX_TAG_TRACK_NAME, username + "--" 
						+ formatterCompleteDateTime.format(new Date(someRecords.get(0).getTime()))
						+ "-" 
						+ formatterCompleteDateTime.format(new Date(someRecords.get(someRecords.size() - 1).getTime())));
	...
	/*public static String create(final ArrayList<Location> locations, final ArrayList<RecordedWayPoint> someWayPoints, String username) throws IllegalArgumentException {
		...
		sb.append(GPX_TAG_TRACK);
		f.format(GPX_TAG_TRACK_NAME, username + "--" 
						+ formatterCompleteDateTime.format(new Date(locations.get(0).getTimeStamp()).getTime())
						+ "-" 
						+ formatterCompleteDateTime.format(new Date(locations.get(locations.size() - 1).getTimeStamp()).getTime()));
		sb.append(GPX_TAG_TRACK_SEGMENT);

		for (RecordedGeoPoint rgp : locations) {
